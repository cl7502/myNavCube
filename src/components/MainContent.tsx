...
function HorizontalLayout() {
   const { tags, categories, selectedCategoryId, settings, setEditingTag } = useStore();
   ...
   const renderCat = (catId: string, depth: number) => {
     ...
     const catInfo = categories.find(c => c.id === catId);
      ...
      if (myTags.length === 0 && subCats.length === 0 && catId !== 'root') return null;
     ...
     return (
       <div key={catId} className="mb-6 pl-2" style={{ marginLeft: depth > 0 ? '24px' : '0' }}>
          ...
          <Droppable droppableId={`drop-tag-horiz-${catId}`} direction="horizontal" type="TAG">
            {(provided) => (
              <div 
                ref={provided.innerRef} 
                {...provided.droppableProps} 
                className={`grid ${myTags.length > 0 ? 'mb-4' : ''} ${myTags.length === 0 ? 'min-h-[40px]' : ''}`} 
                style={{ 
              ...
              >
                {myTags.map((tag, i) => (
                     <TagCard key={tag.id} tag={tag} layout={settings.tagLayout} index={i} colorBlock="blue-500" onEdit={setEditingTag} />
                  ))}
...
const renderSubVert = (catId: string, showHeader: boolean = true) => {
       ...
       const catInfo = categories.find(c => c.id === catId);
        ...
        if (myTags.length === 0 && subCats.length === 0) return null;
            ...
            {tagChunks.length > 0 ? (
              <div className="flex gap-4 flex-wrap">
                {tagChunks.map((chunk, chunkIndex) => (
                  <Droppable key={`${catId}-chunk-${chunkIndex}`} droppableId={`drop-tag-vert-${catId}-${chunkIndex}`} type="TAG">
                     {(provided) => (
                        <div ref={provided.innerRef} {...provided.droppableProps} className={`grid ${chunk.length === 0 ? 'min-h-[40px]' : ''}`}
                          style={{ 
                            gap: `${settings.tagLayout.spacingY}px ${settings.tagLayout.spacingX}px`,
  gridTemplateColumns: `repeat(${effectiveTagsPerColumn}, max-content)`
                          }}>
                         {chunk.map((tag, i) => <TagCard key={tag.id} tag={tag} layout={settings.tagLayout} index={chunkIndex * effectiveTagsPerColumn + i} colorBlock="green-500" onEdit={setEditingTag} />)}
                         {provided.placeholder}
                       </div>
                     )}
                   </Droppable>
                 ...
                 <Droppable droppableId={`drop-tag-vert-${catId}-0`} type="TAG">
                   {(provided) => (
                ...
                return (
                  <div className="flex gap-4 flex-wrap">
                    {rootTagChunks.map((chunk, chunkIndex) => (
                       <Droppable key={`root-chunk-${chunkIndex}`} droppableId={`drop-tag-vert-root-col-${chunkIndex}`} type="TAG">
                         {(provided) => (
                           <div ref={provided.innerRef} {...provided.droppableProps} className="grid gap-2 min-h-[100px]"
                             style={{ 
                              ...
                              {chunk.map((tag, i) => <TagCard key={tag.id} tag={tag} layout={settings.tagLayout} index={chunkIndex * effectiveTagsPerColumn + i} colorBlock="gray-500" onEdit={setEditingTag} />)}
                              {provided.placeholder}
                           </div>
                         )}
                       </Droppable>
...
<div className={`flex items-center gap-2 ${tags.some(t => t.category_id === c.id) ? 'mb-3' : 'mb-1'} group`}>
...
function TagCard({ tag, layout, index, colorBlock, onEdit, ...rest }: { tag: Tag; layout: any; index: number; colorBlock: string; onEdit?: (tag: Tag) => void; [key: string]: unknown }) {
  ...
  return (
<Draggable draggableId={`tag-${tag.id}`} index={index}>
      {(provided) => (
        <div
          ref={provided.innerRef}
          ...
          className={clsx(
            "bg-white border rounded-lg p-2 flex items-center gap-3 shadow-sm transition-all group overflow-hidden border-l-4 hover:border-blue-300 hover:shadow-md cursor-move",
        ...
        >
          <div
            ...
            style={{ color: tag.icon_color || layout.iconColor || '#6b7280' }}
